...
export const SEAPORTS: readonly (Seaport & { country: string; region: string })[] = [
  ...
  },
  {
    ...
    country: 'Canada',
    region: 'North America',
    location: { lng: -123.11, lat: 49.29 },
    teu: 3_530_000,
  },
  {
    code: 'TRAMB',
    name: 'Ambarlı (Istanbul)',
    countryCode: 'TR',
    country: 'Türkiye',
    region: 'Europe',
    location: { lng: 28.69, lat: 40.97 },
    teu: 3_100_000,
  },
  {
    code: 'MAPTM',
  ...
  return cached(key, 20, async () => {
    try {
      ...
      const feed = raw.data ?? {
        vessels: [],
        total: 0,
        attribution: 'AIS relay',
        fetchedAt: new Date().toISOString(),
      };
      const vessels = bbox
        ? feed.vessels.filter((v) => bboxContains(bbox, v.position))
        : feed.vessels;
